...
const ErrorCode = {
    no_error                    : 'Ошибок нет',
    not_found_left_operand      : 'Не найден левый операнд',
    not_found_right_operand     : 'Не найден правый операнд',
    not_found_operand           : 'Не найден операнд',
    unexpect_symbol             : 'Неожиданный символ',
    not_found_open_parenthese   : 'Не найдена открывающая скобка',
    not_found_close_parenthese  : 'Не найдена закрывающая скобка'
};
...
export function is_valid_expression( expr = "" )
{
    ...
    for(let idx = 0; idx < expr.length; ++idx)
    {
        ...
        if (code & ClassCode.dot) has_dot = true;
        if (has_dot) {
            if (code == ClassCode.dot) 
                return { index: idx, error: ErrorCode.unexpect_symbol };
            if (code != ClassCode.digit) has_dot = false;
        }
        ...
        if (code & (ClassCode.open_q | ClassCode.close_q)) has_quantity = !has_quantity;
...
{
    // SetBinary
    if (is_set_binary(symbol)) return ClassCode.set_area | ClassCode.set_binary;
    // SetUnary
    //if (is_set_unary(symbol)) return ClassCode.set_area | ClassCode.set_unary;
    // NumBinary
    if (is_num_binary(symbol, prev)) return ClassCode.num_area | ClassCode.num_binary;
    // NumUnary
    if (is_num_unary(symbol, prev)) return ClassCode.num_area | ClassCode.num_unary;
    // Set
    if (is_variable(symbol)) return ClassCode.set_area | ClassCode.variable;
    // Digit
    if (is_digit(symbol)) return ClassCode.num_area | ClassCode.digit;
    // Dot
    if (is_dot(symbol)) return ClassCode.num_area | ClassCode.dot;
    // CloseCmpl
    if (is_close_cmpl(symbol)) return ClassCode.set_area | ClassCode.close_cmpl;
    // OpenCmpl
    if (is_open_cmpl(symbol)) return ClassCode.set_area | ClassCode.open_cmpl;
    // CloseQ
    if (is_close_q(symbol, hasQ)) return ClassCode.set_area | ClassCode.close_q;
    // OpenQ
    if (is_open_q(symbol, hasQ)) return ClassCode.num_area | ClassCode.open_q;
    // CloseBrk
    if (is_close_brk(symbol)) return ClassCode.close_brk;
    // OpenBrk
    if (is_open_brk(symbol)) return ClassCode.open_brk;
