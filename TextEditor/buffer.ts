...
import { charCount, eraseFromString, findOverlineEnd, insertToString, validationStringIndex, validationStringRange } from "./utils.js";

export class Buffer extends BufferBase {
    caret : Caret;

    inserted_callback : Function;
    erased_callback : Function;

    constructor(inserted_callback : Function = undefined, 
                erased_callback : Function = undefined) {
        super('');
    ...
    getSelected() : string {
        if (!this.caret.hasSelect()) return '';
    ...
    static normalizeOverline(str : string) : string {
        let level = 0;
        let result = '';
        for (const char of str) {
            if (!Buffer.isValidChar(char)) continue;
            if (char === '{') ++level;
            if (char === '}') {
                if (level === 0) continue;
                --level;
    ...
    removeChar() : void {
        ...
        if (this.caret.left() === this.data().length) return;
        this.removePart(this.caret.left(), this.caret.left() + 1);
    }

    removeCharBefore() : void {
        if (this.caret.hasSelect()) {
            this.removeSelect();
            return;
        }
        if (this.caret.left() === 0) return;
    ...
    removePart(start : number, end : number) : void {
    ...
    removeOverlines(start : number, end : number) : number {
        validationStringRange(this.data(), start, end);
        for(let idx = start; idx < end; ++idx) {
            if (this.data()[idx] === '{') {
                const overline_end = this.removeOverline(idx);
                if (overline_end === -1) return -1;
                if (overline_end < end) --end;
                --idx; --end;
            }
        }
        return end;
    ...
    removeOverline(start : number) : number {
