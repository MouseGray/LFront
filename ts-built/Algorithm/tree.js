    ...
    for (let it = 0; it < data.length; ++it) {
        ...
        if (isNumber(char)) {
            try {
                ...
                root.addProjections(Array.from(new Array(number_end + 1 - it), (val, index) => index + it));
                it = number_end;
            }
            catch (error) {
        ...
        if (char === '-') {
            ...
            if (it === 0 || isOperation(data[it - 1])) {
                ...
                while (!stack.empty()) {
                    if (priority('+') > priority(stack.top().value))
                        break;
        ...
        if (isBinaryOperation(char)) {
            while (!stack.empty()) {
                if (priority(char) > priority(stack.top().value))
                    break;
                stack.top().attach(root);
                root = stack.top();
                stack.pop();
