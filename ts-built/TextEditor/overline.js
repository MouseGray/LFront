...
export class Overline {
    ...
    static getOverlineConfig(text, start) {
        let level = 0;
        let max_level = 0;
...
export class OverlineMetrics {
    ...
    static getOverlinesMetrics(context, text, overlines, offsetY) {
        const result = [];
        for (const overline of overlines)
            result.push(OverlineMetrics.getOverlineMetrics(context, text, overline, offsetY));
        return result;
    }
    static getOverlineMetrics(context, text, overline, offsetY) {
        const result = new OverlineMetrics();
        result.startX = renderutils.stringMetrics(context, text.substring(0, overline.start)).width;
        result.endX = result.startX + renderutils.stringMetrics(context, text.substring(overline.start, overline.end)).width;
